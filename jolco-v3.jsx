      ...
      const processRow = () => {
        ...
        for (let xi = 0; xi < xValues.length; xi++) {
          ...
          if (heatXVar === "poPremiumPct") {
            ...
            inp.fxFactor = (inp.jpyUsdBase ?? JPY_USD_BASE) / xVal;
          ...
          if (heatYVar === "poPremiumPct") {
            ...
            inp.fxFactor = (inp.jpyUsdBase ?? JPY_USD_BASE) / yVal;
...
function IrrWaterfallChart({ R }) {
  ...
  const irrBps = (R.blendedIRR ?? 0) * 10000;
  const netProfit = R.jolcoProfit;  // sum of all CFs — most accurate denominator
  const totalStreams = R.totalStream1 + R.totalStream2 + R.totalStream3;

  // Proportional attribution: bps_i = blendedIRR_bps * (stream_i / totalStreams)
  // Handles negatives correctly — shares still sum to 1 regardless of sign.
  const mkBps = (stream) => totalStreams !== 0 ? irrBps * (stream / totalStreams) : 0;

  const bars = [
    { label: "① Hire Spread", sub: "Charter cash yield, net of brokerage & debt service",
      bps: mkBps(R.totalStream1), dollar: R.totalStream1, color: "#9ece6a" },
    { label: "② Tax Shield",  sub: "Depreciation losses → reduced JP investor tax",
      bps: mkBps(R.totalStream2), dollar: R.totalStream2, color: "#bb9af7" },
    { label: "③ Residual/PO", sub: "PO exercise proceeds, net of remaining debt & cap gains tax",
      bps: mkBps(R.totalStream3), dollar: R.totalStream3, color: "#e0af68" },
    { label: "Blended IRR",   sub: "All three streams combined",
      bps: irrBps, dollar: netProfit, color: "#7aa2f7", isTotal: true },
  ];
  ...
  React.useEffect(() => {
    ...
    // x-axis range: include negatives
    ...
    ctx.strokeStyle = "#3b4261";
    ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(zeroX, T - 4); ctx.lineTo(zeroX, H - B + 4); ctx.stroke();
    ...
    bars.forEach((bar, i) => {
      ...
      if (bar.isTotal) {
        // Background track for total bar
        ctx.fillStyle = "#292e42";
        ctx.fillRect(zeroX, y, Math.max(1, bx - zeroX), barH);
      }

      // Bar — diverging: positive goes right, negative goes left from zero
      const barLeft  = Math.min(zeroX, bx);
      const barWidth = Math.max(2, Math.abs(bx - zeroX));
      ctx.fillStyle = bar.bps < 0
        ? "#f7768e" + (bar.isTotal ? "" : "cc")   // red for negative streams
        : bar.color + (bar.isTotal ? "" : "cc");
      ctx.fillRect(barLeft, y, barWidth, barH);

      // Value label — outside the bar end, away from zero
    ...
    ctx.textAlign = "center";
    const tickStep = xRange > 4000 ? 500 : xRange > 1500 ? 200 : 100;
  ...
  return (
    <div ref={containerRef} style={{ width: "100%", position: "relative" }}>
      <canvas ref={canvasRef} width={width} height={canvasH}
      ...
      {hovered && (
        <div style={{
          ...
          background: "#1a1b26", border: "1px solid #3b4261", borderRadius: 8,
          padding: "10px 14px", fontSize: 11, fontFamily: F, zIndex: 999,
          pointerEvents: "none", minWidth: 230, boxShadow: "0 4px 20px rgba(0,0,0,0.5)",
        }}>
          <div style={{ fontWeight: 700, color: "#c0caf5", marginBottom: 4, fontSize: 12 }}>{hovered.label}</div>
          <div style={{ color: "#565f89", fontSize: 10, marginBottom: 8 }}>{hovered.sub}</div>
          <div style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: "3px 10px" }}>
            <span style={{ color: "#565f89" }}>Dollar return</span>
            <span style={{ color: hovered.dollar >= 0 ? "#9ece6a" : "#f7768e", fontWeight: 700 }}>
              {hovered.dollar >= 0 ? "+" : ""}${(hovered.dollar / 1e6).toFixed(2)}M
            </span>
            <span style={{ color: "#565f89" }}>IRR contribution</span>
            <span style={{ color: hovered.bps >= 0 ? "#9ece6a" : "#f7768e", fontWeight: 700 }}>
              {hovered.bps >= 0 ? "+" : ""}{hovered.bps.toFixed(0)} bps
            </span>
            {!hovered.isTotal && (
              <>
                <span style={{ color: "#565f89" }}>% of total profit</span>
                <span style={{ color: "#a9b1d6" }}>
                  {totalStreams !== 0 ? ((hovered.dollar / totalStreams) * 100).toFixed(1) + "%" : "—"}
                </span>
              </>
            )}
          </div>
        ...
        {tab === "deal" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
            ...
            <div style={C}>
              ...
              <Inp label="JPY/USD Rate (¥)" value={jpyUsdRate} onChange={setJpyUsdRate} unit="¥" step={1} min={80} max={250}
                help={`Tax shield FX factor: ${JPY_USD_BASE}/${jpyUsdRate} = ${(JPY_USD_BASE/jpyUsdRate).toFixed(3)}× vs base ${JPY_USD_BASE}`} />
        ...
        {tab === "sensitivity" && (
          ...
          <SensitivityTab
            ...
            onCellClick={(cell) => {
              ...
              else if (heatXVar === "jpyUsdRate") setJpyUsdRate(cell.xVal);
              ...
              else if (heatYVar === "jpyUsdRate") setJpyUsdRate(cell.yVal);
